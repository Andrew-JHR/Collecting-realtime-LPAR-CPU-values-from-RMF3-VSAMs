...
*-------------------------------------------------------------------*
* Updated : Dec 08, 2015 by Andrew Jan                          C151208
* Output an extra 1-byte field to tell how many online CPUs     C151208
*-------------------------------------------------------------------*
         PRINT OFF
         LCLA  &REG
...
*--------------------------------------------------------*
          ...
          MVC   MINTIME,SSHGOMNT save the current mintime       C151208
...
*
          ...
          LM    R2,R3,CPUG3_LOGITI logical cpu times
          M     R2,=F'10000'      multiply 100 for percentage
          DL    R2,CPUG3_NUMPRC+4 total cpu times
          CVD   R3,WORKD          convert to packed decimal
          MVC   LOGCPU,ED_MASK    logical cpu %
          ED    LOGCPU,WORKD+5    logical cpu %
          LM    R2,R3,CPUG3_PHYSTI physicalucpumtimes
          M     R2,=F'10000'      multiply 100 for percentage
          DL    R2,CPUG3_NUMPRC+4 total cpu times
          CVD   R3,WORKD          convert to packed decimal
          MVC   PHYCPU,ED_MASK    physical cpu %
          ED    PHYCPU,WORKD+5    physical cpu %
          L     R10,CPUG3_NUMPRC   to calculate how many cpu    C151208
          L     R11,CPUG3_NUMPRC+4 for the divide instruction   C151208
          DL    R10,=F'1000000'    million minisec -> second    C151208
          DL    R10,MINTIME        mintime                      C151208
          CVD   R11,WORKD          binary to packed decimal     C151208
          UNPK  CPUNUM,WORKD+7(1)  zone decimal                 C151208
          OI    CPUNUM,X'F0'       make it readable             C151208
...
PHYCPU   DS   CL7
         DC   C'%,'
CPUNUM   DS   CL1                cpu #                          C151208
         DC   C','               delimiter                      C151208
         ORG
*
...
MINTIME  DS   F                  mintime got for the sshg3      C151208
*
WORKD     DS   D                 work
*
...
*
         END
/*
//G.RMFVSAM DD DISP=SHR,DSN=SYS1.RMFIII.DS12
//G.PRINT  DD  SYSOUT=*,LRECL=50
//G.WARN   DD  SYSOUT=*
//
